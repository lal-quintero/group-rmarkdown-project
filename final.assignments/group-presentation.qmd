...
test_results %>%
  gt() %>%
  tab_header(
    title = "Whisk Class Hotelling's T² Test Results"
  ) %>%
  cols_label(
...
ggplot(pca_var, aes(x = PC_num, y = Variance)) +
  ...
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 0.0816, ymax = 1.178, alpha = 0.2, fill = "coral") +
...
dist_matrix3 <- as.dist(1 - cor(t(scale(logged.c.whisky[-1]))))
...
manh_c_matrix %>%
  ...
  gt(rownames_to_stub = TRUE) %>%
  tab_header(
    title = "Manhattan Hierarchical Clustering"
  ) %>%
  tab_stubhead(label = "Predicted") %>%
  cols_label(
    Counterfeit = "Counterfeit",
...
euc_c_matrix %>%
  ...
  gt(rownames_to_stub = TRUE) %>%
  tab_header(
    title = "Euclidian Hierarchical Clustering"
  ) %>%
  tab_stubhead(label = "Predicted") %>%
  cols_label(
    Counterfeit = "Counterfeit",
...
```{r}
kmetric <- confusion_metric(kmean_c_matrix)
PAMmetric <- confusion_metric(pam_c_matrix)
manhmetric <- confusion_metric(manh_c_matrix)
eucmetric <- confusion_metric(euc_c_matrix)

clustering_comparison <- data.frame(
  Method = c("K-means", "PAM", "Manhattan HC", "Euclidean HC"),
  Overall_Accuracy = c(kmetric$globalqual_table$OAcc,
                       PAMmetric$globalqual_table$OAcc,
                       manhmetric$globalqual_table$OAcc,
                       eucmetric$globalqual_table$OAcc),
  Average_Accuracy = c(kmetric$globalqual_table$AAcc,
                       PAMmetric$globalqual_table$AAcc,
                       manhmetric$globalqual_table$AAcc,
                       eucmetric$globalqual_table$AAcc),
  F1_Macro = c(kmetric$globalqual_table$F1_m,
               PAMmetric$globalqual_table$F1_m,
               manhmetric$globalqual_table$F1_m,
               eucmetric$globalqual_table$F1_m),
  TNR_Macro = c(kmetric$globalqual_table$TNR_m,
                PAMmetric$globalqual_table$TNR_m,
                manhmetric$globalqual_table$TNR_m,
                eucmetric$globalqual_table$TNR_m),
  F1_Micro = c(kmetric$globalqual_table$F1_mu,
               PAMmetric$globalqual_table$F1_mu,
               manhmetric$globalqual_table$F1_mu,
