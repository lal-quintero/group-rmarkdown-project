...
Chemometric methods: A Potential Tool in the Identification of Counterfeits
...
## Whisky Samples {background-image="barrels.jpeg" background-opacity="0.3"}
...
plot(kmean.w.sil3, main = "Silhouette Plot (K = 3)", border = NA)
plot(kmean.w.sil4, main = "Silhouette Plot (K = 4)", border = NA)
```
...
- LDA - as used in @shand2017multivariate - can be used for counterfeit detection if lower fidelity is desired, or for initial exploration of data
...
- No method of cluster analysis incorrrectly predicted provenance whiskies to be counterfeit, or vice versa

- Grain/blend whiskies provide the most confusion in the model, with a higher likelihood of being misclassified as provenance, and counterfeits being misclassified as grain/blend
...
- The rate of grain/blend false negatives and positives, and the moderate to weak clustering performance observed between regional and grain/blend whiskies suggest more or different data may be needed to truly discern provenance. This could include:

  - Introducing the age of whisky as an added explanatory variable
  - Taking samples of metals and compounds introduced into the whisky during the aging process into account - the aging process may be more distinguishable than regionality
    - The main separator between classes is the inclusion/exclusion of specific elements eg an excess of sulphur in counterfeits, and a higher presence of copper, rubidium, etc, in provenance whiskies

- @shand2017multivariate noted that this methodology should be used with caution due to the unbalanced design of the experiment, so further studies should look to increase the proportion of counterfeit samples included in the analysis
