...
## Overview {background-image="casks.JPG" background-opacity="0.2"}

::: {style="font-size: 80%;"}
Multivariate Analysis of Scotch Whisky by Total Reflection X-ray Fluorescence and
Chemometric methods: A Potential Tool in the Identification of Counterfeits (Shand et al., 2017)
:::


::: {style="font-size: 65%;"}
- Shand et al. (2017) aimed to potentially determine authenticity and provenance of scotch via it's elemental composition using a novel technique, TXRF
- TXRF detection limits are 3 to 4 orders more sensitive than XRF, is more affordable and can quantify trace elements including metals and non-metals
- Chemometric sampling and multivariate analysis using XTRF and has been applied to wine and other spirits for authentication and origin discrimination, though not to scotch
- Elemental constitution is influenced by many factors including grain and water composition, production processes and maturation time

:::

[Sources: @bunnahabhain2005; @kew2016chemical; @shand2017multivariate; @roullier2020influence]{style="font-size: 0.5em;"}

## Aims {background-image="casks.JPG" background-opacity="0.2"}

- To establish literature and data driven clusters for Scotch differentiation, particularly for discriminating against counterfeits
- To determine if results are consistent across methods, and thus suitability of scotch XTRF sampling to cluster analysis
- Quantitatively compare results to Shand et al. (2017) 


## Whisky Samples {background-image="casks.JPG" background-opacity="0.2"}
...
- 32 Whiskies, 7 Classes:
  - 5 Counterfeits
- Primarily Grain Composed (often 60-80%):
  - 8 Blends
  - 2 Grains
- Malt Composed and regional:
  - 2 Highland
  - 2 Lowland
  - 4 Island
  - 9 Speyside
...
[Sources: @storrie1962scotch; @bunnahabhain2005; @bower2016scotch; @kew2016chemical; @shand2017multivariate; @SWACWG2021]{style="font-size: 0.4em;"}



## Trace Elements {background-image="barrels.jpeg" background-opacity="0.3"}
...
::: {style="font-size: 80%;"}
- 11 Trace Elements (Mg/L)
- Associated with whisky production
- Concentrations generally above TRXF detectable limits
- Some evidence has shown Cu capable of distinguishing malt whiskies from blends
:::
 
[Sources: @bunnahabhain2005; @shand2017multivariate]{style="font-size: 0.4em;"}
...
surprise_df2 %>%
  gt() %>%
  tab_header(
    title = "Summary of Surprising Observations"
  ) %>%
  cols_label(
...
- Mean vector ($\mu$) tests for groups with likely differences
...
- **Literature:** 
 - Notes most blended whiskies are 60-80% grain
 - Malts frequently distinguishable from grains via chemometric analysis
 - Counterfeits can be distinguished from additives using spectrometry and congener detection 

- **Data:**
 - Mean vectors varied significantly where was expected
 

[Sources: @storrie1962scotch; @adam2002investigations: @mcintyre2011detection; @bower2016scotch; @kew2016chemical; @aylott2022investigation;  @SWACWG2021]{style="font-size: 0.5em;"}
...
::: {style="font-size: 70%;"}
- Generated 3 predictive aggregated classes from the original 7
 - **Provenance** (Lowland, Highland, Speyside, Island)
 - **Blends & Grains** (Blended and Grain)
 - **Counterfeits** 
:::
...
- Maximum correlations weaker than other groups
- $n= 10$
:::
...
ggplot(pca_var, aes(x = PC_num, y = Variance)) +
  ...
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 0.08163043, ymax = 1.17769561, alpha = 0.2, fill = "coral") +
...
bMpc<- boxM(prov.count12[, 1:3],prov.count12$group)
...
bMc.bg <- boxM(bg.count12[, 1:3],bg.count12$group)
...
bMp.bg <-boxM(bg.prov[, 1:3],bg.prov$group)
...
boxm_results <- tibble(
  Comparison = c(
    "Global Test",
    "Provenance vs Counterfeit",
    "Grain/Blend vs Counterfeit",
    "Grain/Blend vs Provenance"
  ),
  Chi_Sq = c(21.706, 11.457, 14.066, 14.066),
  df = c(12, 6, 6, 6),
  p_value = c(0.04095, 0.07523, 0.02891, 0.02891)
)
...
boxm_results %>%
  gt() %>%
  tab_header(
    title = md("**Box's M-test for Homogeneity of Covariance Matrices**")
  ) %>%
  fmt_number(
    columns = vars(Chi_Sq, p_value),
    decimals = 3
  ) 
...
::: {style="font-size: 50%;"}
- **Elbow plot** showing k-means clustering results for $k = 1$–$10$, using Euclidean distance and the Hartigan–Wong algorithm (50 starts, 100 iterations)
...
plot(hc_whisky.w,
     main = "Euclidian Distance Dendrogram",
     xlab = "Whisky Index",
     ylab = "Height (Distance)",
     hang = -1,                    # Align labels at bottom
     labels = pc_scores.v$Label)  # Use descriptors as labels
...
plot(hc_whisky4,
     main = "Manhattan Distance Dendrogram",
     xlab = "Whisky Index",
     ylab = "Height (Distance)",
     hang = -1,                    # Align labels at bottom
     labels = pc_scores.v$Label)  # Use descriptors as labels
...
plot(hc_whisky3,
     main = "Correlation (1-r) Distance Dendrogram",
     xlab = "Whisky Index",
     ylab = "Height (Distance)",
     hang = -1,                    # Align labels at bottom
     labels = pc_scores.v$Label)  # Use descriptors as labels
...
- **Strong agreement across clustering methods** indicates robustness of the XTRF approach for counterfeit detection in field applications using cluster analysis
...
- TXRF as a novel means of counterfeit detection of Scotch whisky appears to be a sound method, with a reasonable degree of success and accuracy

- Results comparable to LDA and better than euclidean hierarchical clustering as used in Shand et al. (2017)

- Care must be taken with the selection of appropriate distances and number of clusters, as there are varying results and levels of accuracy in both counterfeit and grain/blend discrimination

## Predictions and Classification

- No method of cluster analysis incorrectly predicted provenance whiskies to be counterfeit, or vice versa

- Grain & blend whiskies provide the most confusion in the model, with a higher likelihood of provenance misclassification to, and being misclassified as counterfeits

- This confusion suggests counterfeits passing as grain/blend whiskies may be more likely to go undetected in TXRF analysis, making it a lower risk, lower reward option for counterfeiters

## Further Exploration

- The rate of grain/blend false negatives and positives, and the clustering performance observed between regional and grain/blend whiskies suggest more or different data may be needed to truly discern provenance. This could include:

  - Introducing the age of whisky as an added explanatory variable
  - Taking samples of other metals and compounds introduced into the whisky during the aging process into account

## Sample Size

- Shand et al. (2017) noted that this methodology should be used with caution due to the unbalanced design of the experiment, so further studies should look to increase the proportion of counterfeit samples included in the analysis
